...
from utils.distributions_sampling_utils import *
...
def plot_distributions(params, resolution=1000):
    ...
    # Plot marginal distributions in X
    ...
    plt.plot(x_vals, f_signal * g_s_vals + (1 - f_signal) * g_b_vals,
             label='Total $f_X(X)$', color='black', linewidth=2)
    plt.xlabel('$X$')
    ...
    # Plot marginal distributions in Y
    plt.subplot(1, 2, 2)
    ...
    plt.plot(y_vals, f_signal * h_s_vals + (1 - f_signal) * h_b_vals,
             label='Total $f_Y(Y)$', color='black', linewidth=2)
    plt.xlabel('$Y$')
    plt.ylabel('Probability Density')
    ...
    plt.colorbar(label='Joint PDF $f(X, Y)$')
    plt.xlabel('$X$')
    plt.ylabel('$Y$')
